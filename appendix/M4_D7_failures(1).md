...
**复现**：`M4_guardrail_test.py` 用例 1 直接对 `Guardrails.check_turns` 做单元验证；集成上，一个 20 步相同调用的脚本会触发 `step_cap`（第 9 步，MAX_TURNS=8）。
...
**复现**：`M4_guardrail_test.py` 用例 6，脚本在同一 turn 内两次调用 `get_claim` 相同参数 → 触发 `duplicate_action`。
